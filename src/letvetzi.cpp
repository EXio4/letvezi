...
namespace Letvetzi {
    ...
    namespace GameState {
        ...
        Type::Type(Game::Resolution res_p, Position player_p, Game::sdl_info* sdl_inf, Persistent* persistent)
        ...
        void Type::add_bg_particle(int16_t start_y) {
        ...
        void Type::maybe_add_enemy(double prob) {
            double  type  = bg_particles_gen.enemy_type (bg_particles_gen.random_eng);
            if (type > 1-prob) add_enemy();
        };

        void Type::add_bullet(Velocity vel) {
        ...
        void Type::restart_game(bool first) {
        ...
        void Type::show_highscores() {
            game_state = HighScores;
            SDL_StartTextInput();
        };
        void Type::back_to_main_menu() {
            SDL_StopTextInput();
            restart_game();
        };

        void Type::show_credits() {
            game_state = Credits;
            credit_text_pos = Position(res.width/4, res.height + 32);
        };

        void Type::with_menu(std::function<void(Menu&)> fn) {
            game_state = GameMenu;
            menu.current = 0;
            menu.pressed = 0;
            menu.opts.clear();
        ...
        void Type::start_boss(int boss_id) {
        ...
        void Type::with_new_entity(std::function<std::shared_ptr<Entity::Type>(Entity::Name)> fn) {
                Entity::Name e = last_entity;
                ++last_entity;
                ent_mp[e] = fn(e);
        }
        void Type::add_points(uint32_t p) {
            points += p;
            til_boss -= p;
            if (til_boss <= 0) {
        ...
        void Type::add_life(unsigned int li) {
            lives += li;
            if (lives == 0) {
